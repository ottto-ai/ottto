...
- Cloud-session upload is `cloud_session_observations.v1` only: content-free metadata with opaque provider entity keys and no provider-day usage or fabricated task cost. Identical entity polls emit no observation payload; a bounded health-only heartbeat is eligible hourly so backend freshness remains below the two-hour stale threshold without touching Sessions GOLD.

...
- Cloud-session upload is `cloud_session_observations.v1` only: content-free metadata with opaque provider entity keys and no provider-day usage or fabricated task cost. Every batch is explicitly `snapshot` or `heartbeat`. Heartbeats are always observation-empty and `snapshot_complete: false`. A snapshot is complete only after terminal-cursor enumeration without page, item, deadline, cancellation, parse, or provider-error truncation; partial bounded snapshots can never authorize absence deletion and report degraded health so UI freshness cannot present their coverage as complete. Backend v1 has no coverage-limited error category, so this uses its existing coarse `provider_error` category while `snapshot_complete` remains the authoritative absence signal. At least one unchanged complete snapshot is replayed per active UTC day so the backend can drain stale rows in bounded work. Other identical polls emit no observation payload; a bounded health-only heartbeat is eligible hourly so backend freshness remains below the two-hour stale threshold without touching Sessions GOLD.

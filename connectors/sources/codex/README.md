...
- `identity_probe`: subprocesses `codex doctor --json` and reads the plain
  ...
  decodes the local ID-token claims inside each exact `CODEX_HOME` to hash the
  ChatGPT user and active workspace; token bytes and raw identifiers never
  leave the process or enter an upload.

...
Pi remains an official Ottto app/source with provider-aware attribution. Pi is currently accepted as a local-only source: Ottto reads local session and route metadata, but does not require Pi live telemetry to mark the source healthy.

Collectors:

- `local_sessions`: reads local Pi session JSONL files through `ottto-locald` and uploads aggregate local usage snapshots.
- `live_batches`: disabled by default. It documents the future first-party normalized live usage path, but live telemetry is not an acceptance requirement for Pi.
...
Raw prompts, responses, command output, local paths, provider credentials, and API keys must not be uploaded by these collectors. Fresh live telemetry matching must not block Pi setup or stable release acceptance while Pi is in local-only mode.

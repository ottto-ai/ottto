...
fn start_builtin_relays(daemon: &LocalDaemon) {
    // Say out loud which input decided Claude attribution capture. An operator
    // whose LaunchAgent environment was regenerated by `brew upgrade` can read
    // one line instead of inferring the answer from missing selector fields.
    let capture = ottto_service::snapshots::claude_attribution_capture_status();
    eprintln!(
        "Claude attribution capture {} (source: {})",
        if capture.enabled {
            "enabled"
        } else {
            "disabled"

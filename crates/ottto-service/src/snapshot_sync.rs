...
mod tests {
    ...
    #[test]
    #[serial(source_manifests)]
    fn exhausted_claude_authority_is_terminal_residue_but_retry_pending_stays_unsettled() {
        let _source_manifests = SourceManifestTestGuard::new();
        ...
        let retry_pending = claude_usage_authority_census_counts(
            "retry-pending-claude-authority-receipt",
            &[("unproven_terminal", 3), ("retry_pending", 1)],
        );
        assert!(!retry_pending.census_complete);
        assert!(retry_pending.scan_cap_hit);
        assert_eq!(retry_pending.ownership_incomplete_file_count, 3);
        assert_eq!(retry_pending.terminal_ownership_incomplete_file_count, 3);
        let receipt = captured_terminal_receipt_for_source_under_hint(
            SnapshotSource::ClaudeCode,
            retry_pending,
            CollectorState::Success,

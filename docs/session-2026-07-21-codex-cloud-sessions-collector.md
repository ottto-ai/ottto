...
- Added local versioned grant storage with immediate pause/revoke APIs. Grant
  scopes are HMAC-fingerprinted before persistence; no provider credentials are
  stored. Revocation is rechecked before transport.
...
Validation: `cargo test -p ottto-service cloud_sessions --lib` exercises content
leak prevention, idempotency/no-op, disabled/revoked grants, the revoke-before-
send race, deferred-transport no-call behavior, pagination/cursor persistence,
and circuit breaking. Local load evidence is an in-memory 60-task, three-page
fixture under the hard item/page caps; it completes below one second and never
stores its cursors. A repeated semantic digest writes no second payload.

...
if [[ "$min_protocol_version" != "12" ]]; then
  fail "Stable manifest min_protocol_version must be 12"
fi
...
if local_platform.get("protocol_version") != 12:
    die("stable-candidate RC evidence local_platform.protocol_version must be 12")

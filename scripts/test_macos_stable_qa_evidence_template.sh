...
jq -e \
  ...
  '
    ...
    and (.install_owners[] | select(.owner == "homebrew") | .local_platform.protocol_version == 15)
...
grep -q "min_protocol_version must be 15" /tmp/stable-qa-template-stale-protocol.out
